export 默认 async function handler(req, res) {
  ...
  try {
    ...
    let fetchedDynamicPages = 0;

    while (
      videos.length < VIDEO_LIMIT &&
      fetchedDynamicPages < MAX_DYNAMIC_PAGES
    ) {
      const dynamicRes = await fetch(buildDynamicUrl(offsetDynamicId), {
        headers: commonHeaders,
      });

      if (!dynamicRes.ok) {
        throw new Error(`B站动态服务器连接失败`);
      }
      ...
      const nextOffset = dynamicData.data?.next_offset;

      if (
        dynamicData.data?.has_more === 0 ||
        dynamicData.data?.has_more === false ||
        !nextOffset ||
        String(nextOffset) === offsetDynamicId
      ) {
        break;
      }
